...
function BuildInvitationModal({ order }) {
  ...
  const [extraData, setExtraData] = useState({
    groomAr: order.groomAr || "", brideAr: order.brideAr || "",
    messageEn: order.messageEn || "Together with their families",
    messageAr: order.messageAr || "معاً مع عائلتيهما",
    venue: order.venue || `${order.ceremonyPlace}, ${order.city}`,
    venueAr: order.venueAr || "",
    parentsEn: order.parentsEn || "", parentsAr: order.parentsAr || "",
    ...
    partyMapUrl: "",
    music: order.music || "",
    musicStart: order.musicStart ? Number(order.musicStart) : null,
    musicEnd: order.musicEnd ? Number(order.musicEnd) : null,
    video: order.video || "",
    videoStart: order.videoStart ? Number(order.videoStart) : null,
    videoEnd: order.videoEnd ? Number(order.videoEnd) : null,
    muteVideo: false,
    rsvpDeadline: order.rsvpDeadline || "",
    registryWishMoneyAcc: order.registryWishMoneyAcc || "",
    registryWishMoneyPhone: order.registryWishMoneyPhone || "",
    registryLink1: order.registryLink1 || "",
    registryLink2: order.registryLink2 || "",
    registryBeneficiary: order.registryBeneficiary || "",
    registryIban: order.registryIban || "",
    registryBic: order.registryBic || "",
    registrySubtitle: order.registrySubtitle || "",
    hideCeremony: false,
    hideTimeline: false,
    askEventAttendance: false,
    tl0: order.tl0 || "5:00 PM",    tl0loc: order.tl0loc || "",
    tl1: order.tl1 || "7:00 PM",    tl1loc: order.tl1loc || "",
    tl2: order.tl2 || "11:00 PM",   tl2loc: order.tl2loc || "",
